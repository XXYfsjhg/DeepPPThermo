...
class RNN(nn.Module):
    def __init__(self, length_feature1, length_feature2, length_feature3, length_feature4, length_feature5):
        ...
        self.fc_out = nn.Linear(in_features=600, out_features=1, bias=True)
        ...
        self.sa1 = ScaledDotProductAttention(d_model=1, d_k=1, d_v=1, h=4)
    ...
    def forward(self, x, x1, x2, x3, x4, x5):
        ...
        x_sa = torch.cat((sa, x1, x2, x3, x4, x5), dim=1)
...
positive_csv_DPC = 'D:\\pro_design\\doc2vec+lstm\\doc2vec+lstm\\input\\csv\\CKSAAP.csv'

negative_csv_CKSAAP = 'D:\\pro_design\\doc2vec+lstm\\doc2vec+lstm\\input\\csv\\negativeAAC.csv'
positive_csv_CKSAAP = 'D:\\pro_design\\doc2vec+lstm\\doc2vec+lstm\\input\\csv\\AAC.csv'

negative_csv_CTriad = 'D:\\pro_design\\doc2vec+lstm\\doc2vec+lstm\\input\\csv\\negativeAPAAC.csv'
positive_csv_CTriad = 'D:\\pro_design\\doc2vec+lstm\\doc2vec+lstm\\input\\csv\\APAAC.csv'
...
positive_csv_AAC = 'D:\\pro_design\\doc2vec+lstm\\doc2vec+lstm\\input\\csv\\PAAC.csv'
...
for rand_seed in list_random:
    ...
    rnn = RNN(length_feature1, length_feature2, length_feature3, length_feature4, length_feature5)
    optimizer = optim.Adam(rnn.parameters(), lr=1e-3)  # sgd
    ...
    for epoch in range(50):  # 10iter   1min
        ...
        train_DPC_0 = dataset.DataLoader(data_DPC_train_0, batch_size=20, shuffle=True)
        train_DPC_1 = dataset.DataLoader(data_DPC_train_1, batch_size=20, shuffle=True)
        train_DPC_0 = dataload2list(train_DPC_0)
        train_DPC_1 = dataload2list(train_DPC_1)

        train_CKSAAP_0 = dataset.DataLoader(data_CKSAAP_train_0, batch_size=20, shuffle=True)
        train_CKSAAP_1 = dataset.DataLoader(data_CKSAAP_train_1, batch_size=20, shuffle=True)
        train_CKSAAP_0 = dataload2list(train_CKSAAP_0)
        train_CKSAAP_1 = dataload2list(train_CKSAAP_1)

        train_CTriad_0 = dataset.DataLoader(data_CTriad_train_0, batch_size=20, shuffle=True)
        train_CTriad_1 = dataset.DataLoader(data_CTriad_train_1, batch_size=20, shuffle=True)
        train_CTriad_0 = dataload2list(train_CTriad_0)
        train_CTriad_1 = dataload2list(train_CTriad_1)

        train_ASDC_0 = dataset.DataLoader(data_ASDC_train_0, batch_size=20, shuffle=True)
        train_ASDC_1 = dataset.DataLoader(data_ASDC_train_1, batch_size=20, shuffle=True)
        train_ASDC_0 = dataload2list(train_ASDC_0)
        train_ASDC_1 = dataload2list(train_ASDC_1)

        train_AAC_0 = dataset.DataLoader(data_AAC_train_0, batch_size=20, shuffle=True)
        train_AAC_1 = dataset.DataLoader(data_AAC_train_1, batch_size=20, shuffle=True)
        ...
        for iteration in range(Iters):
            ...
            pred = rnn(x, x_DPC, x_CKSAAP, x_CTriad, x_ASDC, x_AAC)
            loss = criteon(pred, y)
